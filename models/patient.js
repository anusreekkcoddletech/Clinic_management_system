...
async function getPatientsbyDate(yesterday, today, tomorrow) {
    ...
    try {
        if (yesterday && today && tomorrow) {
            const qr = `SELECT * FROM patients WHERE  DATE(created) IN (CURRENT_DATE(), DATE_ADD(CURRENT_DATE(), INTERVAL 1 DAY),DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY) )ORDER BY created`
            const patientsbyDate = await db.query(qr)
            return patientsbyDate
        } 
        else if ( yesterday  && !today &&  tomorrow) {
            const qr1 = `SELECT * FROM patients WHERE DATE(created) IN ( DATE_ADD(CURRENT_DATE(), INTERVAL 1 DAY),DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY))`
            const patientsbyDate = await db.query(qr1)
            return patientsbyDate
        } 
        else if ( yesterday && today && !tomorrow)  {
            const qr2 = `SELECT * FROM patients WHERE DATE(created) IN (CURRENT_DATE(),DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY))`
            const patientsbyDate = await db.query(qr2)
            return patientsbyDate
        }
         else if (!yesterday && today && tomorrow  ) {
            const qr3 = `SELECT * FROM patients WHERE DATE(created) IN (CURRENT_DATE(), DATE_ADD(CURRENT_DATE(), INTERVAL 1 DAY))`
            const patientsbyDate = await db.query(qr3)
            return patientsbyDate
        }
         else if ( yesterday && !today && !tomorrow ) {
            const qr4 = `SELECT * FROM patients WHERE DATE(created) =DATE_SUB(CURRENT_DATE(), INTERVAL 1 DAY)`
            const patientsbyDate = await db.query(qr4)
            return patientsbyDate
        }
        else if ( !yesterday && today && !tomorrow  ) {
            const qr5 = `SELECT * FROM patients WHERE DATE(created) =CURRENT_DATE()`
            const patientsbyDate = await db.query(qr5)
            return patientsbyDate
        }
        else if ( !yesterday && !today && tomorrow  ) {
            const qr6 = `SELECT * FROM patients WHERE DATE(created) = DATE_ADD(CURRENT_DATE(), INTERVAL 1 DAY)`
            const patientsbyDate = await db.query(qr6)
            return patientsbyDate
        }
        else if ( !yesterday && !today && !tomorrow  ) {
            const qr7 = `SELECT * FROM patients`
